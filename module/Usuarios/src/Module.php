...
class Module {
    ...
    public function getServiceConfig() { //service manager
        return [
            'factories' => [
                AuthenticationService::class => InvokableFactory::class,
                Login::class => function ($sm) {    //uso de clase login.php
                    $dbAdapter = $sm->get(AdapterInterface::class);
                    $authService = $sm->get(AuthenticationService::class);
                    return new Login($dbAdapter, $authService); //valores a login, db adapter
                },
